...
Reconciled from the research note `docs/Research/vettrackg4g5slice13verified.md` §9 (external store requirements verified against Apple/Google policy, not the repo; the note is kept in the repo as a working reference, so THIS section is the authoritative version). These are **submission prerequisites with hard calendar deadlines** that the original ladder never captured. Several are already past or imminent as of 2026-08-09 — treat as an owner action list, one repo issue each. **Re-read 2026-08-22:** no row here has been re-verified against its source since it was written. Relative durations are deliberately not used below — they were accurate on 2026-08-09 and are the reason this table read as less urgent than it was. **P4** binds on **2026-08-31** and is already satisfied (see its row). **P5** is the live one: its 12-tester / 14-consecutive-day closed test cannot finish by 2026-08-31 even if it starts today, because no AAB has ever been uploaded (`docs/release-config.md` §C) and the 14 days only begin once Google approves the release and 12 testers have opted in. ~~P5's **OWNER: confirm account type** question has been open since this section was written and is still unanswered anywhere in this repo~~ — ~~**answered 2026-08-22: personal account, created after 2023-11-13, so the requirement APPLIES** (see the P5 row). That answer moved the bottleneck rather than removing it: the upload is a half-hour of work, and finding twelve people with real Android devices who will use the app for fourteen continuous days is what now sets the date.~~ **Re-opened and then settled on 2026-08-22:** the first answer described a Play account the owner reports as restricted following a failed identity verification, which will not be used. The replacement is `danerez5@gmail.com`, and the owner confirms it is a **personal** account — so the requirement **applies** after all, and the only thing the re-opening changed was the calendar. That account's identity verification is **in flight with Google** (owner report 2026-08-22; Google quoted days). The bottleneck is therefore neither the upload nor the testers but Google's verification queue, which nothing in this repo shortens. What is and is not blocked behind it is tabulated in `docs/release-config.md` §C. Each deadline below should be re-checked against its official source before acting, since store policies shift. *Corrected 2026-08-22 (this sentence previously read ~~"(linked per row)"~~):* no row carried a link — the sources live in `docs/VetTrack-Readiness-Review.md` §D (each with the date it was fetched) and in the research note cited above. Only the two rows re-checked on 2026-08-22 carry a link inline; the rest deliberately do not, because a link this pass did not open is not a citation.
...
| P4 | Target API 36 (Android 16) | Google | **2026-08-31** ([source](https://support.google.com/googleplay/android-developer/answer/11926878), re-checked 2026-08-22) | extension possible to 2026-11-01. **Checked 2026-08-22: already satisfied, with nothing to change.** `node_modules/react-native/gradle/libs.versions.toml` declares `targetSdk = "36"` and `compileSdk = "36"`, and that catalog is what Expo's root-project Gradle plugin reads (`getVersionOrDefault("targetSdk", "35")` — the `35` is only a fallback for a missing catalog). The generated `android/app/build.gradle` takes it via `rootProject.ext.targetSdkVersion`, and this repo installs no `expo-build-properties` override. The risk is therefore a regression, not a gap: anything that pins an override, or an SDK downgrade, silently drops the target. |
| P5 | Closed test — **12 testers × 14 consecutive days** ([source](https://support.google.com/googleplay/android-developer/answer/14151465), re-checked 2026-08-22 — the 14 days start only once Google approves the release AND 12 testers have opted in, and since April 2026 Google also rejects on insufficient tester *engagement*, not just count) | Google | rolling | **APPLIES — not started; the account exists but cannot publish yet.** ~~**OWNER: confirm account type.**~~ ~~**APPLIES — owner confirmed 2026-08-22: the Play account is personal and was created after 2023-11-13.**~~ *Corrected twice on 2026-08-22 — first to UNKNOWN, then back:* the original confirmation described the `moridan5020@gmail.com` account, which failed identity verification, became heavily restricted, and will not be used. The replacement developer account is `danerez5@gmail.com`, and the owner confirms it is **personal** — so this row applies, exactly as the first answer said, for a different account. (An **organization** account would be exempt, but requires a D-U-N-S number and incorporation documents; that route was not taken.) The account's **identity verification is in flight with Google** and until it clears, no upload is possible — so this row is blocked one step further upstream than "recruit twelve testers". The clock has not started: no AAB has ever been uploaded (`docs/release-config.md` §C). **The track matters and is easy to get wrong** — internal testing does **not** count toward this requirement; only a **closed** test does, which is `track: "alpha"` in Play's API and therefore in `eas.json`. That value was `internal` until 2026-08-22, so a submit would have produced the Play App Signing key and started no clock at all — a failure that only surfaces weeks later, when production access is refused. **`alpha` is now not merely the safe default but the required destination**, since the personal-account answer is settled: reverting to `internal` would produce a signing key, start no clock, and hide the failure until production access is refused weeks later. Real sequence: upload to the closed track → Google approves the release → ≥12 testers opt in → 14 continuous days of genuine use → only then apply for production. |
...
| P9 | `DELETE /api/users/delete-account` wired in the RN app | both | at submit | **DONE — direct in-app deletion, not a link-out.** `DeleteAccountCard` (`src/features/account/AccountSection.tsx`) runs a two-step confirm and then calls `DELETE /api/users/delete-account` in-app, which satisfies Apple 5.1.1(v)'s in-app *initiation* requirement outright. Google separately requires a **web** deletion resource as well — that half is P6, and it is not discharged by this row. *Corrected 2026-08-22 (this cell previously read ~~"Deferred-to-G5 per Slice 12 seam"~~):* it shipped ten days before this correction, in commit `c48d3993` — `src/lib/api/account.ts` exports `accountApi.deleteAccount`, and `src/features/account/AccountSection.tsx` renders the two-step-confirm `DeleteAccountCard` that calls it. The contract that cell describes — self-scoped `DELETE`, no id, no body, and the coded `reason`s the screen branches on — is pinned by `src/lib/__tests__/api.account.test.ts`. |
...
**Critical path (hard deadlines, none in the original ladder):** P2, P3, P4, P5. P4 is now satisfied; P5's entry condition was briefly recorded as settled and is not. *Corrected 2026-08-22, twice in one day — first from ~~"hence the account-type question is the single most time-sensitive owner decision"~~, then from ~~"that question is answered; recruiting the twelve testers is the single most time-sensitive owner action"~~:* *and once more, to this:* **the single most time-sensitive owner action on the Android lane is now a queue nobody here controls** — Google's identity verification of `danerez5@gmail.com`, in flight as of 2026-08-22. Twelve testers *are* needed (the account is personal), so the one thing worth doing during the wait is lining up twelve people with real Android devices, which is the part that actually sets the date. Everything else on the Android lane — upload, closed track, production access — queues behind the verification. Everything **off** that lane, including the entire iOS submission, does not: see `docs/release-config.md` §C for the split.
