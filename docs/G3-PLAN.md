...
| P5 | Closed test — **12 testers × 14 consecutive days** ([source](https://support.google.com/googleplay/android-developer/answer/14151465), re-checked 2026-08-22 — the 14 days start only once Google approves the release AND 12 testers have opted in, and since April 2026 Google also rejects on insufficient tester *engagement*, not just count) | Google | rolling | **UNKNOWN — applicability is re-opened and this row is neither satisfied nor cleared.** ~~**OWNER: confirm account type.**~~ ~~**APPLIES — owner confirmed 2026-08-22: the Play account is personal and was created after 2023-11-13.**~~ *Corrected later on 2026-08-22:* that confirmation described the `moridan5020@gmail.com` account, which the owner reports failed identity verification and became heavily restricted; it will not be used. The developer account is being re-established under a different Google account and **has not been registered yet**, so its type is not yet a fact about this project. A **personal** account created after 2023-11-13 is subject to this requirement; an **organization** account is exempt, but requires a D-U-N-S number and incorporation documents. Recording this as unknown rather than as either answer is deliberate: both a false APPLIES and a false exempt would mis-size the Android lane by weeks. The clock has not started: no AAB has ever been uploaded (`docs/release-config.md` §C). **The track matters and is easy to get wrong** — internal testing does **not** count toward this requirement; only a **closed** test does, which is `track: "alpha"` in Play's API and therefore in `eas.json`. That value was `internal` until 2026-08-22, so a submit would have produced the Play App Signing key and started no clock at all — a failure that only surfaces weeks later, when production access is refused. **`alpha` stays correct whichever account type lands**, so the re-opening above changes no configuration: a closed track is the right first destination either way, and reverting to `internal` would restore the trap for the personal case while buying the organization case nothing. Real sequence: upload to the closed track → Google approves the release → ≥12 testers opt in → 14 continuous days of genuine use → only then apply for production. |
