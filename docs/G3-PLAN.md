...
| P9 | `DELETE /api/users/delete-account` wired in the RN app | both | at submit | **DONE.** Apple 5.1.1(v) wants in-app *initiation*; Google allows the in-app path to link to the web resource. *Corrected 2026-08-22 (this cell previously read ~~"Deferred-to-G5 per Slice 12 seam"~~):* it shipped ten days before this correction, in commit `c48d3993` — `src/lib/api/account.ts` exports `accountApi.deleteAccount`, and `src/features/account/AccountSection.tsx` renders the two-step-confirm `DeleteAccountCard` that calls it. The contract that cell describes — self-scoped `DELETE`, no id, no body, and the coded `reason`s the screen branches on — is pinned by `src/lib/__tests__/api.account.test.ts`. |
