...
| P5 | Closed test — **12 testers × 14 consecutive days** ([source](https://support.google.com/googleplay/android-developer/answer/14151465), re-checked 2026-08-22 — the 14 days start only once Google approves the release AND 12 testers have opted in, and since April 2026 Google also rejects on insufficient tester *engagement*, not just count) | Google | rolling | **APPLIES — owner confirmed 2026-08-22: the Play account is personal and was created after 2023-11-13.** ~~**OWNER: confirm account type.**~~ The clock has not started: no AAB has ever been uploaded (`docs/release-config.md` §C). **The track matters and is easy to get wrong** — internal testing does **not** count toward this requirement; only a **closed** test does, which is `track: "alpha"` in Play's API and therefore in `eas.json`. That value was `internal` until this was confirmed, so a submit would have produced the Play App Signing key and started no clock at all — a failure that only surfaces weeks later, when production access is refused. Real sequence: upload to the closed track → Google approves the release → ≥12 testers opt in → 14 continuous days of genuine use → only then apply for production. |
...
**Critical path (hard deadlines, none in the original ladder):** P2, P3, P4, P5. P4 is now satisfied and P5's entry condition is settled, so the path narrowed rather than shortened. *Corrected 2026-08-22 (this line previously ended ~~"hence the account-type question is the single most time-sensitive owner decision"~~):* that question is answered. **Recruiting the twelve testers is the single most time-sensitive owner action**, because it is the only item here with a clock that runs outside this repo and cannot be shortened by any change to it.
