...
- Results CSV schema: `app,device,run,metric,value_ms,frames_total,frames_over_budget,frame_source,participant_id,order,outcome,failure_reason` (`participant_id` = `owner` for every row under this revision; `frame_source` = `js`/`ui` for O1/O2 rows, empty otherwise; `outcome` = `ok`/`failed`).
