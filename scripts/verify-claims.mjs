 ...
 * This project documents itself unusually well and nothing checks the
 * documentation. `AGENTS.md` and `SCAFFOLD-PLAN.md` named op-sqlite as the
 * persistence engine for months; the repo has never carried that dependency.
 * The correction, when it came, was hand-written next to the false line. Recent
 * commit subjects on main read "four false claims caught in audit" and "fix six
 * plan steps that would fail or pass falsely as written". The defect class is
 * known; the response to it has been a human re-reading everything.
 *
 ...
 * scripts/verify/run.js, which the jest wrapper requires, so the gate that
 * blocks a PR and the command a human runs can never disagree.
 *
 * MODES
