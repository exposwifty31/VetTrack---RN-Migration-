 ...
 * A gate that re-enters this tooling (`npm test`, `npm run test:ci`, a bare
 * `jest`, any `verify:*` script) is rejected outright rather than executed. That
 * is a configuration error with a clear message, not a hang. The predicate lives
 * in scripts/verify/claims.js so it can be handed a bad command in a test.
 */
...
const { resolveGitBinary } = require("./verify/git-facts.js");
...
const { isReentrantGate, GATE_TOKEN, gateInvocation, createOutputCollector, terminationProblem } = require("./verify/claims.js");
...
fs.writeFileSync(
  ...
  `${JSON.stringify(
    {
      $comment:
        "Written by `npm run verify:evidence`. Read by the claim gate: a document that says a gate is green must be backed by a PASS here, on this tree. Do not hand-edit.",
      treeHash,
      dirty,
      startedAt,
