 ...
 *
 * WHAT THIS BLOCKS
 * `scripts/verify-claims.mjs` makes the repository's own prose self-checking:
 ...
 * `release-config-checks.test.ts` uses, for the same reason.
 *
...
const rules = require("../../scripts/verify/claims.js") as Rules;
const scan = require("../../scripts/verify/scan.js") as Scan;
const { verify } = require("../../scripts/verify/run.js") as {
  verify(options?: { root?: string; now?: string; enforceEvidence?: boolean }): VerifyResult;
};
const { globToRegExp, createFacts } = require("../../scripts/verify/facts.js") as {
...
const gitFacts = require("../../scripts/verify/git-facts.js") as {
...
const fingerprint = require("../../scripts/verify/fingerprint.js") as {
...
const ENGINE_DIR = path.join(__dirname, "..", "..", "scripts", "verify");
...
describe("layer 1 — what is written exists", () => {
  ...
  it("refuses a dependency version the manifest does not carry", () => {
    // The defect this repo actually shipped: the frozen-stack line said
    // "Gesture Handler 3.x" while package.json carried ~2.32.0.
  ...
  it("refuses an ABSENCE claim the dependency set contradicts", () => {
    // The op-sqlite defect, as a test: AGENTS.md asserted for months that no
    // SQLite package existed, and nothing checked it. Had a SQLite dependency
    // been present, this is the refusal that would have caught it on day one.
  ...
  it("refuses a documented npm script the manifest does not define", () => {
  ...
  it("refuses a package reference the manifest does not declare", () => {
    // `@clerk/clerk-expo` was swapped for `@clerk/expo`; the docs kept the old name.
...
describe("layer 2 — what is written as done was in fact done", () => {
  ...
  it("proves a REBASE-merged pull request through the ledger, not around it", () => {
    // PR #2 of this repo was rebase-merged: it has no merge commit, and reading
    // that as "never landed" is a false alarm against a true sentence. The
...
describe("layer 3 — the declared gates actually ran, and passed, on this tree", () => {
  ...
  it("accepts a current, green report — and stays quiet outside CI when there is none", () => {
    ...
    // `npm test` for that is a false alarm aimed at normal work.
...
describe("the engine refuses what it says it refuses", () => {
  it("rejects every spelling of a gate that would re-enter the test suite", () => {
    ...
    for (const command of [
      "npm run typecheck",
      "npm run lint",
      "npm run release:preflight:offline",
  ...
  it("rejects a gate token carrying shell punctuation", () => {
    ...
    for (const token of ["npm", "run", "release:preflight:offline", "--platform=ios", "./bin/x"]) {
  ...
  it("treats `?` in a glob as a literal, not as a quantifier", () => {
    ...
    expect(globToRegExp("src/*.ts", { suffix: true }).test("src/a.ts")).toBe(true);
  });
  ...
  it("refuses an absence claim when the walk could not read a directory", () => {
    ...
    const spy = jest.spyOn(fsModule, "readdirSync").mockImplementation(((
  ...
  it("reads a re-verify recipe with the package manager the repository uses", () => {
    ...
    expect(rules.scriptNameIn("npm run ios")).toBe("ios");
    ...
    expect(rules.scriptNameIn("docs/device-test-w3b.md", "pnpm")).toBeNull();
  });
