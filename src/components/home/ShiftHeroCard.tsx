    ...
    <View className="px-[22px] pt-2.5">
      <View className="rounded-[24px] border border-border bg-surface-raised px-5 py-4">
        <View className="flex-row items-baseline gap-2.5">
          ...
          {subline ? (
            <Text className="font-rubik text-[12.5px] text-muted">{subline}</Text>
          ) : null}
        </View>
            ...
            <View className="mt-3.5 flex-row items-center gap-3 rounded-[16px] border border-border bg-surface px-4 py-2.5">
              <View className="flex-1">
                ...
                <Text className="mt-[1px] font-rubik text-[12px] text-muted">
                  {pendingAdjustment.kind === "extend"
                    ? t("shiftAdjustments.extend")
                    : t("shiftAdjustments.leaveEarly")}
                  {" · "}
                  <Text style={{ writingDirection: "ltr" }}>
                    {clockLabel(pendingAdjustment.requestedEndTime)}
                  </Text>
                </Text>
              </View>
              <PressableScale
                ...
                onPress={() => onCancelAdjustment(pendingAdjustment.id)}
              >
