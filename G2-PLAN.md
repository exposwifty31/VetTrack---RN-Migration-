...
6. **Native run:** `npx expo prebuild --clean` (safe — ios/android are gitignored CNG, 0 tracked files; NFC entitlement regenerates from app.json's plugin) → `npx expo run:ios`. New Arch is mandatory (Reanimated 4 hard-requires it).
...
2. `npx expo install` yielded exactly: reanimated **4.5.1**, worklets **0.10.1** (NOT 0.11.x), gesture-handler **~2.32.x** (NOT 3.x), flash-list **2.0.2**; `npm ls` shows no ERESOLVE.
...
4. **Bisect (step 4): `npx expo start -c` serves the JS bundle GREEN with a trivial worklet Reanimated animation active — no `transformFile`-undefined / transform-worker init crash.**
...
6. `npx expo prebuild --clean && npx expo run:ios` launches on the sim; the worklet animation runs on-thread; no native link errors.
