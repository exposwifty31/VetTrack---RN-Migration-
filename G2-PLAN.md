...
10. **E2E on sim against the chosen backend** (DONE gate — contingent on Step 1 green): drive Scan→search/NFC→FlashList→CheckoutConfirm→server-confirmed toggle, capturing all three instrumentation outputs. **Before the blind test, run the step-2 contract check for real** (scan semantics, 409 shape, undoToken) and verify tag payload on device.
